...
f_inits <- function(chain_id = 1) {
  ...
  list(
    ...
    beta_phi = rnorm(3, 0, .01),
    gam_phi = rnorm(3, 0, .01),
...
trans = stanc(file = '02_computational_modeling/02_other/04_ispt_rb_ct_oe.stan')
...
for(i in names(dl)) {
  ...
  # gp_diag = c(gp_s[ -which(gp_s %in% c('vax_b', 'gam', 'beta')) ],
  #             paste0('vax_b[', 1:4, ']'),
  #             paste0('gam[', 1:3, ']'),
  #             paste0('beta[', 1:3, ']'))
  # 
  ...
  #                write_path = paste0('modeling/stan_diags/ispt_rb_ct_oe_', i))
  # )
  ...
  # list with results
  ispt_oe[[i]] = list(pars = list(p_pars = p_pars,
                                  i_pars = i_pars),
...
rm(list = setdiff(ls(), 'ispt_oe'))
save.image('02_computational_modeling/posteriors/05_ispt_vc_irb_ct_oe.RData')