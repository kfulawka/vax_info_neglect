...
parameters {
  ...
  real beta_phi[3]; // this should be analyzed on 0-1 range
  real gam_phi[3]; // this should be analyzed on 0-1 range
...
transformed parameters {
  ...
  for(s in 1:N) {
    ...
    irb[s] = rb + id_phi[1, s];
    ibeta[s] = Phi_approx(beta_phi[LA[s]] + id_phi[2, s]);
    igam[s] = Phi_approx(gam_phi[PS[s]] + id_phi[3, s]);
...
generated quantities {
  ...
  // LA to monitor
  real <lower = 0, upper = 1> beta[3];
  real <lower = 0> beta_sigma;
  
  // PS to monitor
  real <lower = 0, upper = 1> gam[3];
