...
transformed parameters {
  ...
  for(s in 1:N) {
    ...
    ialpha[s] = exp(alpha_exp + id_sig[1] * id_z[1, s]);
    ibeta[s] = Phi_approx(beta_phi + id_sig[2] * id_z[2, s]);
    igam[s] = Phi_approx(gam_phi + id_sig[3] * id_z[3, s]);
    iphi[s] = Phi_approx(phi_phi + id_sig[4] * id_z[4, s]);
