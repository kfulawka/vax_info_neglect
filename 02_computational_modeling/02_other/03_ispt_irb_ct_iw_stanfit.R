...
# se group indicies
for(g in c('anti', 'neu', 'pro')) {
  
  dl[[g]]$se_g = cbind(p_se1 = c(1, 3, 1, 2, 1, 1, 2, 1),
                       p_se2 = c(1, 3, 1, 2, 1, 1, 2, 3),
                       p_se3 = c(1, 3, 2, 2, 2, 2, 2, 3))
...
trans = stanc(file = '02_computational_modeling/02_other/03_ispt_rb_ct_iw.stan')
...
for(i in names(dl)) {
  ...
  gp_diag = c(gp_s[ -which(gp_s %in% c('vax_b', 'p_e')) ],
              paste0('vax_b[', 1:4, ']'),
              paste0('p_e[', 1:4, ']'))
  ...
  try(stan_diags(stanfit = stanfit,
                 ...
                 ind_p = ip_s,
                 group_p = gp_diag,
                 pairs_p = list(p1 = c(gp_diag[grepl('beta|gam|alpha|phi|rb|lp__', gp_diag)]),
                                p2 = c(gp_diag[grepl('vax', gp_diag)]),
                                p3 = c(gp_diag[grepl('beta|gam|p_e', gp_diag)])),
                 write_path = paste0('modeling/stan_diags/ispt_rb_ct_iw_', i))
  )
  ...
  # list with results
  ispt_iw[[i]] = list(pars = list(p_pars = p_pars,
                               i_pars = i_pars),
  ...
  # clean the env
  rm(stanfit, looE, sampling_info, fit_summary, #performance,
...
rm(list = setdiff(ls(), 'ispt_iw'))
save.image('02_computational_modeling/posteriors/04_ispt_vc_irb_ct_iw.RData')