...
for(i in names(dl)) {
  ...
  try(stan_diags(stanfit = stanfit,
                 ...
                 write_path = paste0('02_computational_modeling/stan_diags/ispt_irb_ct_', i))
  )
  ...
  # list with results
  ispt[[i]] = list(pars = list(p_pars = p_pars,
                               i_pars = i_pars),
...
rm(list = setdiff(ls(), 'ispt'))
save.image('02_computational_modeling/posteriors/01_ispt_vc_irb_ct.RData')